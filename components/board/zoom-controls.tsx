...
export function ZoomControls() {
    ...
    return (
        <div className="absolute bottom-4 left-4 flex items-center gap-1 bg-white dark:bg-neutral-800 p-1 rounded-lg shadow-md border border-neutral-200 dark:border-neutral-700 pointer-events-auto">
             <Button variant="ghost" size="icon" onClick={() => handleZoom(-0.1)} title="Zoom arrière">
                <Minus className="h-4 w-4" />
            </Button>
            
            <button 
                onClick={handleReset}
                className="w-12 text-xs font-mono font-medium text-center hover:bg-neutral-100 dark:hover:bg-neutral-700 rounded py-1"
                title="Réinitialiser le zoom"
            >
                {Math.round(camera.zoom * 100)}%
            </button>

            <Button variant="ghost" size="icon" onClick={() => handleZoom(0.1)} title="Zoom avant">
