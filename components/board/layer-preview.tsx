...
function getSvgPathFromPoints(points: number[][]) {
    ...
    for (let i = 0; i < points.length; i++) {
        ...
        if (isNextMove) { d += `M ${point[0]} ${point[1]}`; isNextMove = false; } else { d += `L ${point[0]} ${point[1]}`; }
    }
    return d;
}
...
export const LayerPreview = memo(({ id, onLayerPointerDown, onLayerResizePointerDown, onLayerRotatePointerDown, onChange, selectionColor }: LayerPreviewProps) => {
  ...
  if (layer.type === "Path") {
      if (layer.width === 0 || layer.height === 0) { isDrawing = true; } 
          ...
          pathScaleX = layer.width / (maxX - minX || 1); pathScaleY = layer.height / (maxY - minY || 1);
      }
  }
    ...
    >
      ...
      {layer.type === "Rectangle" && (
         <>
             ...
             <foreignObject width={layer.width} height={layer.height} style={{ overflow: 'visible', pointerEvents: 'none' }}>
                <div style={wrapperStyle}><div ref={editableRef} contentEditable={isEditing} onBlur={handleBlur} onPointerDown={onTextPointerDown} style={{...editableStyle, padding: '8px'}} /></div>
             </foreignObject>
         </>
      )}
      {layer.type === "Triangle" && (
      ...
      {layer.type === "Ellipse" && (
         <>
             ...
             <foreignObject width={layer.width} height={layer.height} style={{ overflow: 'visible', pointerEvents: 'none' }}>
                 <div style={{...wrapperStyle, alignItems: 'center'}}><div ref={editableRef} contentEditable={isEditing} onBlur={handleBlur} onPointerDown={onTextPointerDown} style={{...editableStyle, width: '80%', padding: '4px'}} /></div>
             </foreignObject>
         </>
      )}
      {layer.type === "Note" && (
        <foreignObject width={layer.width} height={layer.height} style={{ overflow: 'visible' }}>
             <div className="w-full h-full relative shadow-md flex flex-col" style={{ backgroundColor: layer.fill ? layer.fill : "#fef3c7", borderRadius: cornerRadius }}>
                <div style={wrapperStyle}><div ref={editableRef} contentEditable={isEditing} onBlur={handleBlur} onPointerDown={onTextPointerDown} style={{...editableStyle, padding: '8px', fontFamily: layer.fontFamily ? fontMap[layer.fontFamily] : fontMap["font-handwriting"], color: layer.textColor || "#1f2937"}} /></div>
             </div>
        </foreignObject>
      )}
      {layer.type === "Text" && (
