...
import { Trash2, AlignLeft, AlignCenter, AlignRight, AlignVerticalJustifyCenter, AlignVerticalJustifyStart, AlignVerticalJustifyEnd, Bold, Italic, Underline, Ban, Copy, Lock, Unlock } from "lucide-react";
...
export const SelectionTools = memo(({ camera }: SelectionToolsProps) => {
    ...
    const selectionInfo = useStorage((root) => {
        ...
        let isLocked = true; // Start true and set false if any is unlocked
    ...
    return (
        <div className="absolute top-20 right-4 md:right-auto md:left-1/2 md:-translate-x-1/2 flex flex-row items-center gap-2 p-2 bg-white dark:bg-neutral-800 rounded-lg shadow-xl border border-neutral-200 dark:border-neutral-700 pointer-events-auto overflow-x-auto max-w-[95vw] no-scrollbar" onMouseDown={preventFocusLoss}>
            ...
            <div className="flex items-center gap-1">
                <Button variant="ghost" size="icon" onClick={toggleLock} title={isLocked ? "Déverrouiller" : "Verrouiller"} className={`h-8 w-8 hover:bg-neutral-100 shrink-0 ${isLocked ? "text-red-500" : ""}`}>
