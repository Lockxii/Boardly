...
export function PencilToolbar() {
  const {
    canvasState, lastUsedColor, setLastUsedColor,
    pencilThickness, setPencilThickness, pencilTool,
  } = useCanvasStore();
  ...
  return (
    <div className="absolute top-20 right-4 md:right-auto md:left-1/2 md:-translate-x-1/2 flex flex-row items-center gap-2 p-2 bg-white dark:bg-neutral-800 rounded-lg shadow-xl border border-neutral-200 dark:border-neutral-700 pointer-events-auto">
      <div className="flex items-center gap-2 px-2">
        {pencilTool === "draw" ? <Pencil className="h-4 w-4 text-neutral-500" /> : <Eraser className="h-4 w-4 text-neutral-500" />}
      ...
      {pencilTool === "draw" && (
        <>
          ...
          <div className="flex items-center gap-2">
            ...
            <ColorPresets onSelect={(c) => setLastUsedColor(c)} />
          </div>
        </>
      )}
    </div>
