...
export function LayerCommentsPanel() {
  ...
  const layerId = selection.length === 1 ? selection[0] : null;
  if (!layerId) return null;

  const comments = layerComments[layerId] || [];
  ...
  return (
    <div className="absolute top-22 left-4 w-72 max-h-80 flex flex-col rounded-xl border border-neutral-200 dark:border-neutral-700 bg-white/95 dark:bg-neutral-900/95 backdrop-blur-md shadow-xl z-40 pointer-events-auto">
      <div className="flex items-center gap-2 border-b border-neutral-200 dark:border-neutral-700 px-3 py-2">
        ...
        <span className="ml-auto text-xs text-neutral-400">{comments.length}</span>
      </div>
          ...
          comments.map((c) => (
            <div key={c.id} className="rounded-lg bg-neutral-50 dark:bg-neutral-800 p-2 text-xs">
              <div className="flex items-start justify-between gap-2">
                ...
                {!readOnly && (
                  <button type="button" className="text-neutral-400 hover:text-red-500" onClick={() => deleteLayerComment(layerId, c.id)}>
